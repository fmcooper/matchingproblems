...
class Model:
    ...
    def get_results(self, short_or_long):
        ...
        results += 'cost: ' + self._get_cost_string(pair_assignments) + '\n'
        if short_or_long == Output_type.LONG:
            results += '# the sum of squares of ranks of matched students\n'
        results += ('cost_sq: ' + self._get_cost_sq_string(pair_assignments) + 
            '\n')
        if short_or_long == Output_type.LONG:
            results += '# the highest rank of a matched student\n'
        results += 'degree: ' + self._get_degree_string(pair_assignments) + '\n'   
        if short_or_long == Output_type.LONG:
            results += '# the number of students gaining their 1st, 2nd, 3rd\n' 
            results += '# choice project etc\n'
        results += ('profile: ' + self._get_profile_string(pair_assignments) + 
            '\n')
        if short_or_long == Output_type.LONG:
        ...
        results += ('max_lec_abs_diff: ' + 
            self._get_max_lec_abs_diff(pair_assignments) + '\n')
        if short_or_long == Output_type.LONG:
        ...
        results += ('sum_lec_abs_diff: ' + 
            self._get_sum_lec_abs_diff(pair_assignments) + '\n\n')
    ...
    def _get_cost_string(self, pair_assignments):
    ...
    def _get_cost_sq_string(self, pair_assignments):
    ...
    def _get_degree_string(self, pair_assignments):
        ...
        return str(max_matched_rank)


    def _get_profile_string(self, pair_assignments):
        """Returns the profile of the matching as a String.
    ...
    def _get_max_lec_abs_diff(self, pair_assignments):
        """Returns a string containing the maximum lecturer absolute difference.
